...
**Importante:** essas credenciais existem apenas para demonstração acadêmica. Em produção, devem ser substituídas.
...
OAuth 2.0 não foi implementado porque a atividade pede apenas a explicação.
...
│   └── vite.config.js
├── postman/
│   └── Sistema-Gestao-Usuarios.postman_collection.json
└── README.md
```

## 15. Observação para entrega
...
O professor pode executar `npm install` em cada pasta.
